...
export const useCheckBoxStyles = createUseStyles({
    ...
    checkBox: ({ disabled, clicked, indeterminate, currentColor }) => ({
        width: "16px",
        height: "16px",
    ...
    indeterminateIcon: ({ disabled, clicked, indeterminate, currentColor }) => ({
...
export const getStyle = (indeterminate, clicked, disabled, currentColor) => {
    if (!indeterminate && clicked) {
        return disabled ? { fill: "rgba(189,184,184,1)" } : { fill: `${currentColor}` }
    } else if (clicked) {
